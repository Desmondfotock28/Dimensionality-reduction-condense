...
def plot_results3(t, x, u, x_a, u_a, xSS, uSS, fignum):
    linewidth = 1.5
    xSSp = xSS
    uSSp = uSS
    ...
    # Plot state trajectories on the first two rows (axes[0, :] and axes[1, :])
    state_labels = ['$x_1$ [rad]', '$x_2$ [m]', '$x_3$ [rad/s]', '$x_4$ [m/s]']
    for i in range(4):
        ax = axes[i // 2, i % 2]  # Distribute the 4 states across the first two rows
        ax.plot(t, xSSp[i] * np.ones_like(t), linewidth=linewidth, label='Reference')
        ax.plot(t, x[:, i][:len(t)], linewidth=linewidth, label='Trajectory')
        ax.plot(t, x_a[:, i], linewidth=linewidth, label='Active Trajectory')
    ...
    # Plot control input trajectories on the third row (axes[2, :])
    ax_u = axes[2, 0]  # Control input plot in the first column of the third row
    ax_u.plot(t, uSSp * np.ones_like(t), linewidth=linewidth, label='Reference')
    ax_u.step(t[:-1][:len(t[:-1])], u[:len(t[:-1])], linewidth=linewidth, where='post', label='Trajectory')
    ax_u.step(t[:-1], u_a, linewidth=linewidth, where='post', label='Active Trajectory')
    ax_u.set_xlabel('$t$ [s]', fontsize=12)
    ax_u.set_ylabel('$u$ [N]', fontsize=12)
    ax_u.grid(True)
    #ax_u.legend()

    # Hide the second column of the third row if there's no data for it
    axes[2, 1].axis('off')
